...
def _create_client() -> tuple[TestClient, StateStore, BookingStore]:
    ...
    return TestClient(app), state_store, booking_store


def test_change_trip_persists_authoritative_state_json() -> None:
    client, state_store, booking_store = _create_client()
    ...
    assert state_store.has("booking-201") is True
    assert state_store.load("booking-201") == payload["persisted_state_json"]
...
def test_fresh_request_restores_state_and_applies_booking_change() -> None:
    ...
    assert state_store.has("booking-202") is True
...
def test_restore_without_saved_itinerary_does_not_apply_change() -> None:
    ...
    state_store.save("booking-203", '{"policies":{},"premise":null,"version":2}')
...
def test_restore_engine_from_persisted_state_round_trips_authoritative_state() -> None:
    client, state_store, _ = _create_client()
    ...
    assert state_store.load("booking-204") == state_json
